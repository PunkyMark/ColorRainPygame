...
class Ball(object):
    ...
    def draw(self, win):
        ...
        if self.left:
            ...
            win.blit(still_ball, (self.x, self.y))
...
def redrawGameWindow():
    win.blit(pixelpos[pixelpos_list], (0, 0))
    score(score_int)
    for rain in rains:
        rain.draw(win)
    ball.draw(win)
...
while not gameExit:
    ...
    if keys[pygame.K_LEFT] and ball.x > ball.vel + 65:
        ball.x -= ball.vel
        ball.left = True
        ball.right = False
    elif keys[pygame.K_RIGHT] and ball.x < 500 - ball.width - ball.vel - 65:
        ball.x += ball.vel
        ball.right = True
        ball.left = False
    else:
